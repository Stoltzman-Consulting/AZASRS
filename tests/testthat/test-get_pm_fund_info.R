...
test_that("get_pm_fund_info() returns matching tibble", {
  ...
  pm_fund_info_benchmarks_head = get_pm_fund_info(add_benchmark = TRUE) %>% head(3)

  expect_equal(pm_fund_info_head, readRDS_test(pm_fund_info_head))
  expect_equal(pm_fund_info_benchmarks_head, readRDS_test(pm_fund_info_benchmarks_head))
