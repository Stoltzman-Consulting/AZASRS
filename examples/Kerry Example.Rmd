...
date_cutoff = as.character(lubridate::today()) #last possible day for data to analyze
```
...
```{r}
# set baseline end date to reference
end_date_base = as.Date(valdate) 

# 1 quarter
start_date = as.character(as.Date(as.yearqtr(end_date_base)) - 1 )

irr_1 = calc_pm_metrics_df(nav_daily, cash_flow_daily, benchmark_daily, start_date, valdate, pm_fund_portfolio, pm_fund_category, pm_fund_id)

irr_2 = calc_pm_metrics_df(nav_daily, cash_flow_daily, benchmark_daily, start_date, valdate, pm_fund_portfolio, pm_fund_category) %>% 
  mutate(pm_fund_id = 'TOTAL')

irr_3 = calc_pm_metrics_df(nav_daily, cash_flow_daily, benchmark_daily, start_date, valdate, pm_fund_portfolio) %>% 
...
```{r}
# 1 year
start_date = end_date_base - lubridate::years(1) + lubridate::days(1)
...
```{r}
# 3 year
start_date = end_date_base - lubridate::years(3) + lubridate::days(1)
...
```{r}
# 5 year
start_date = end_date_base - lubridate::years(5) + lubridate::days(1)
...
```{r}
# 10 year
start_date = end_date_base - lubridate::years(10) + lubridate::days(1)
...
irr_1 = calc_pm_metrics_df(nav_daily, cash_flow_daily, benchmark_daily, start_date, date_cutoff, valdate, pm_fund_portfolio, pm_fund_category, pm_fund_id)

irr_2 = calc_pm_metrics_df(nav_daily, cash_flow_daily, benchmark_daily, start_date, date_cutoff, valdate, pm_fund_portfolio, pm_fund_category) %>% 
  mutate(pm_fund_id = 'TOTAL')

irr_3 = calc_pm_metrics_df(nav_daily, cash_flow_daily, benchmark_daily, start_date, date_cutoff, valdate, pm_fund_portfolio) %>% 
  mutate(pm_fund_id = 'TOTAL', pm_fund_category = 'TOTAL')
...
```{r}
#### Issues!
# what if starting nav == 0 and cash_flow == 0 but there is an ending nav?
# if funded during quarter, no starting nav will be there... but one would be reported on 12/31 --> 
# any nav < 0 should be NA (maybe flag raised and put aside in another table)
# if cash_flow and nav are reported on same first day, drop the cash_flow (consider it already included in nav)
# if cash_flow on last day (where nav is reported) add the cash_flow as part of calc
# 

# nav_daily_backup = nav_daily
# cash_flow_daily_backup = cash_flow_daily
# benchmark_daily_backup = benchmark_daily

nav_daily = nav_daily_backup
cash_flow_daily = cash_flow_daily_backup
benchmark_daily = benchmark_daily_backup




# BCQP check for pcap
# BlkMort has 0 ending val (valdate)
#Vista7 has negative starting nav
```
