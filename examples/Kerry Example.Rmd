...
aggregate_lockdown_metrics = function(dat = nav_cf_joined, grouping_var =  pm_fund_id){
  ...
  df = dat %>%
    ...
    mutate(unrealized_gain_loss_up_to = 
           ...
           pct_of_total_fund_up_to = nav_up_to_end / total_fund_value,
           pct_of_total_fund_after = nav_after_end / total_fund_value,
           estimated_exposure_w_unfunded = nav_after_end + unfunded,
           estimated_exposure_w_unfunded_pct = estimated_exposure_w_unfunded / (total_fund_value + sum(unfunded)))
  return(df)
}
