...
years_quarters = tibble(years    = c(0, 1, 3, 5, 7, 10, 30), 
                       quarters = c(1, 0, 0, 0, 0, 0, 0)) %>%
...
```{r}
years_quarters = tibble(years    = c(0, 1, 3, 5, 10, 70), 
...
df = years_quarters %>%
  mutate(irrs = purrr::pmap(list(start_date, end_date), build_privm_p2p_irr))
...
# db_irr = build_privm_p2p_irr('1900-01-01', '2019-06-30')
# ss_irr = read_csv('examples/pmedf_k.csv')
...
for(i in 1:nrow(years_quarters)){
  ...
  final_data[[i]]$cool_name = yq$cool_namez
}
...
nav = get_pm_nav_daily() %>% as_tibble()
cf = get_pm_cash_flow_daily() %>% as_tibble()
min_date = min(nav$effective_date)
bm = get_benchmark_daily_index(con, return_tibble = FALSE) %>% 
  filter(effective_date >= min_date) %>%
  as_tibble()
pmfi = get_pm_fund_info()
start_date = '2018-12-31'
value_date = '2019-06-30'
pcap_date = '2019-06-30'
```

```{r}

a = build_privm_metrics(pm_fund_category, pm_fund_id, con = con, nav, cf, bm, pmfi, start_date, value_date, pcap_date)
```
