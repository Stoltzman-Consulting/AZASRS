...
#'
#' @export
build_nav_cash_flow_combined = function(pm_fund_portfolio, pm_fund_category, pm_fund_id,
  ...
  if(itd){
    nav = nav_daily %>%
      ...
      dplyr::group_by(pm_fund_portfolio, pm_fund_category, pm_fund_id, effective_date) %>%
  ...
  dat = dplyr::union_all(nav, cf) %>%
    ...
    dplyr::arrange(pm_fund_portfolio, pm_fund_category, pm_fund_id, effective_date) %>%
    dplyr::ungroup() %>%
